...
const URL = "/models/retro_computer.glb?v=13"; // swap this one path to use a different rig
...
export default function PcScene({ active = true }: { active?: boolean }) {
  ...
  useEffect(() => {
    ...
    setScale(window.matchMedia("(max-width: 1024px)").matches ? 2.8 : 2.5);
  }, []);
  ...
  return (
    <Canvas
      ...
      camera={{ position: [0, 2.59, 20], fov: 28 }} // moved camera further back to see full model
      onCreated={({ gl }) => {
